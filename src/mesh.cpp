...
inline LineType classifyLine(std::string_view line) {
  ...
  if (line.size() >= 3 && line[0] == 'v' && line[1] == 't' &&
      (line[2] == ' ' || line[2] == '\t')) {
    return LineType::Texture;
  }
  if (line.size() >= 3 && line[0] == 'v' && line[1] == 'n' &&
      (line[2] == ' ' || line[2] == '\t')) {
...
inline void parseFaceLine(std::string_view s, std::size_t v_seen,
  ...
  for (;;) {
    ...
    std::string_view sn =
        i2 == std::string_view::npos ? std::string_view{} : tok.substr(i2 + 1);
    ...
    std::size_t v =
        (sv.empty() || !okv) ? sentinel : normalizeIndex(iv, v_seen);
    std::size_t t =
        (st.empty() || !okt) ? sentinel : normalizeIndex(it, t_seen);
    std::size_t n =
        (sn.empty() || !okn) ? sentinel : normalizeIndex(in, n_seen);
...
void consumerWork(SPMCQueue<batch *> &queue, consumer_store &store,
  ...
  for (;;) {
    ...
    std::size_t v_seen = b->v_seen;
    std::size_t t_seen = b->t_seen;
    std::size_t n_seen = b->n_seen;
          ...
          if (q != std::string_view::npos) {
            ...
            if (type == LineType::Vertex) {
              ...
              if (!x.empty() && !y.empty() && !z.empty()) {
                const char *p0 = x.data();
                const char *e0 = p0 + x.size();
                const char *p1 = y.data();
                const char *e1 = p1 + y.size();
                const char *p2 = z.data();
                const char *e2 = p2 + z.size();
                float xf = parseFloat(p0, e0);
                float yf = parseFloat(p1, e1);
                float zf = parseFloat(p2, e2);
                store.vertices.push_back(vec3f{xf, yf, zf});
              }
              ++v_seen;
            } else if (type == LineType::Texture) {
              ...
              if (!u.empty()) {
                const char *p0 = u.data();
                const char *e0 = p0 + u.size();
                float uf = parseFloat(p0, e0);

                float vf = 0.0f;
                if (!v.empty()) {
                  const char *p1 = v.data();
                  const char *e1 = p1 + v.size();
                  vf = parseFloat(p1, e1);
                }
              ...
              if (!x.empty() && !y.empty() && !z.empty()) {
                ...
                float yf = parseFloat(p1, e1);
                float zf = parseFloat(p2, e2);
                store.normals.push_back(vec3f{xf, yf, zf});
              }
              ++n_seen;
...
public:
  ...
  bool importObj(void *obj, std::size_t file_size) {
    ...
    std::thread producer([this, obj, file_size]() {
      producerWork(mQueue, obj, file_size, mConfig);
    });
  ...
  bool exportObj(int fd) {
    ...
    const std::size_t nb = this->mBatchArtifacts.size();

    for (std::size_t bid = 0; bid < nb; ++bid) {
      const batch_artifact &a = this->mBatchArtifacts[bid];
      const consumer_store &cs = this->mConsumerStores[a.consumer_id];
...
bool Mesh::importObj(const char *path) {
  ...
  void *obj = mmap(nullptr, file_size, PROT_READ, MAP_PRIVATE, fd, 0);
  close(fd);
  if (obj == MAP_FAILED) {
    return false;
  }
